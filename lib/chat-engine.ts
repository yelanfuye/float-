...
export async function generateOfflineChatCompletion(
    ...
    llmMessages.push({
        role: "system",
        content: "线下输出格式：使用线上聊天的双语格式，原文与翻译用 | 分隔，翻译放在原文后面，或按线上双语分段格式输出；翻译不是正文，不得参与对白识别和语音。只有中文双引号“……”内的文字属于角色对白，原文中的对白必须保留中文双引号。双引号仅用于对白；强调、动作、特殊标记等非对白内容一律使用【】。若输入或草稿误用中文双引号表达非对白含义，生成正文时改写为【】。对白中的语气标签必须沿用线上语音标签逻辑，使用短标签插入对白内部，例如“[开心]句子a，句子b，[低声][犹豫]句子c”；标签只能出现在原文对白内部，不得出现在翻译、描写或【】内容中。没有对白时不要添加语气标签。"
    });
