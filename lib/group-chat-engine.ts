...
export async function generateGroupOfflineChatCompletion(
    ...
    llmMessages.push({
        role: "system",
        content: "线下群聊输出格式：使用线上聊天的双语格式，外语原文与中文翻译用 | 分隔，翻译放在原文后面，或按线上双语分段格式输出；也可将翻译放在原文后方的括号内，或用独立的横线/下划线分隔段。翻译不是正文，不得参与对白识别和语音，TTS 只朗读外语原文。只有角色实际说出口的内容才使用中文双引号“……”或英文直引号。强调、动作、表情、心理、拟态、比喻等非实际发言内容一律使用【】；例如‘她的表情仿佛在说“怎么还来”’必须输出为‘她的表情仿佛在说【怎么还来】’，不能使用对白引号。若输入或草稿误用引号表达非对白含义，生成正文时改写为【】。对白中的语气标签必须沿用线上语音标签逻辑，使用短标签插入外语原文对白内部，例如“[开心]sentence a，[低声][犹豫]sentence b”；标签只能出现在原文对白内部，不得出现在翻译、描写或【】内容中。没有对白时不要添加语气标签。"
    });
