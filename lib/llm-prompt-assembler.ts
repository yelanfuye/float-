        ...
        if (voice?.enableTTS && voice.provider === "ElevenLabs" && voice.model?.trim().toLowerCase() === "eleven_v3") {
            ...
            blocks.push({
                ...
                text: [
                    ...
                    "同一段台词可以在实际语气变化的位置穿插不同标签，也可以连续几句不标；标签放在对应句子之前或自然停顿处。每一句最多使用两个音频标签，通常一个就够；禁止在一句话前扎堆放三个或更多标签。每个标签必须独立使用一对完整英文中括号，例如 [a] [b]，不得合并、嵌套或漏掉括号。不要把标签连续堆在整段开头；需要改变语气时再放到对应句子前。未标记的下一句不一定会自动重置语气。示例（仅演示结构，不照抄）：[语音条:[surprised] 你已经到啦？我还以为要再等一会儿。[playfully] 好，那这次算你赢。]。", 
                    "不要每句必加标签。保持选定音色与人物身份，不主动使用口音、年龄、deep voice、robotic tone、pirate voice 或 x accent 等声线改变指令；不使用 cuts in、overlapping 等模拟多人音轨。默认不用喊叫、强烈戏剧表演。使用正常标点组织节奏，不使用 SSML，不承诺标签能精确控制停顿时长。标签只允许简短英文、空格、逗号、连字符或单引号，不嵌套标签。生成前自行检查：每个句子的标签数量不超过 2 个，并确认每个标签都是独立的 [完整内容] 结构。", 
                    inCall
