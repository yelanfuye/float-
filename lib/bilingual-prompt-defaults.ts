...
export const DEFAULT_OFFLINE_CHAT_BILINGUAL_PROMPT = [
  ...
  "- 对白长度不影响标点格式：无论对白只有几个字还是很长的一整段，只要是角色实际说出口的内容，都必须使用成对的“”或英文双引号，并保持为可生成语音的对白；绝不能因为字数多就改用【】。",
  "- 旁白、动作、表情、环境、心理、拟态、比喻、氛围和事实陈述统一使用【】表达，不得使用中文或英文引号包裹；只有实际对白可以使用成对的“”或英文双引号。",
  "- 如果输出中出现独立成行的长段角色发言，也必须用引号包裹；【】只能用于非对白内容，不能用来包裹任何长度的角色发言。",
...
export const DEFAULT_GROUP_OFFLINE_CHAT_BILINGUAL_PROMPT = [
  ...
  "- 对白长度不影响标点格式：无论对白只有几个字还是很长的一整段，只要是群成员实际说出口的内容，都必须使用成对的“”或英文双引号，并保持为可生成语音的对白；绝不能因为字数多就改用【】。",
