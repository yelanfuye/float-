...
export const DEFAULT_OFFLINE_CHAT_BILINGUAL_PROMPT = [
  "【线下双语与标点规则（仅非中文角色对白使用，中文对白忽略双语规则）】",
  "- 只对 <content> 中角色实际说出口的直接对白生效。",
  "- 旁白、动作、表情、环境、心理、拟态、比喻、氛围和事实陈述统一使用【】表达，不得使用中文或英文引号包裹；只有实际对白可以使用成对的“”或英文双引号。",
  "- 如果角色对白使用非中文语言，请把该对白单独成行，写成“完整原文|对应的简体中文译文”。",
  "- 双语对白必须作为独立段落输出，前后用空行与旁白隔开；不要把旁白和“原文|译文”混在同一段。",
  "- 如果对白本来就是中文，正常输出中文，不要添加 |译文。",
  "- 仅当绑定的语音模型是 eleven_v3 时，才允许在外语对白开头使用一个 ElevenLabs v3 英文方括号语气标签，例如 [whispers]、[angrily, fed up]、[quietly]、[sigh]、[interjecting]、[deep voice]；标签必须位于对应外语原文短句前，不得放进中文译文、旁白、【】或普通中文正文。",
  "- 不要使用中文状态词、（轻声）、语气：温柔、【撒娇】等作为语气标签，也不要把它们写入外语 TTS 正文；不要堆叠多个标签。非 eleven_v3 模型不要输出语气标签。",
  "- 不要改动 <content>、摘要 XML 标签或其他结构标签。",
].join("\n");

export const DEFAULT_GROUP_OFFLINE_CHAT_BILINGUAL_PROMPT = [
  "【群聊线下双语与标点规则（仅非中文角色对白使用，中文对白忽略双语规则）】",
  "- 只对 <content> 中群成员实际说出口的直接对白生效。",
  "- 旁白、动作、表情、环境、心理、拟态、比喻、群体氛围和事实陈述统一使用【】表达，不得使用中文或英文引号包裹；只有实际对白可以使用成对的“”或英文双引号。",
  "- 如果某个角色的对白使用非中文语言，请把该对白单独成行，写成“完整原文|对应的简体中文译文”。",
  "- 双语对白必须作为独立段落输出，前后用空行与旁白或其他动作隔开；不要把旁白和“原文|译文”混在同一段。",
  "- 仅当绑定的语音模型是 eleven_v3 时，才允许在外语对白开头使用一个 ElevenLabs v3 英文方括号语气标签，例如 [whispers]、[angrily, fed up]、[quietly]、[sigh]、[interjecting]、[deep voice]；标签必须位于对应外语原文短句前，不得放进中文译文、旁白、【】或普通中文正文。",
  "- 不要使用中文状态词、（轻声）、语气：温柔、【撒娇】等作为语气标签，也不要把它们写入外语 TTS 正文；不要堆叠多个标签。非 eleven_v3 模型不要输出语气标签。",
