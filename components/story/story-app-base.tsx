...
function storyContentToPlainText(content: string): string {
  if (!content) return "";
  return content
    .replace(/<br\s*\/?>/gi, "\n")
    .replace(/<\/(p|div|li|h[1-6]|blockquote|details|summary)>/gi, "\n")
    .replace(/<li[^>]*>/gi, "• ")
    .replace(/<[^>]+>/g, "")
    .replace(/&nbsp;/gi, " ")
    .replace(/&amp;/gi, "&")
...
function getStoryMessageSearchText(message: StoryMessage): string {
  return [message.rawContent, message.renderedContent, message.storySummary]
    .filter(Boolean)
    .map((value) => storyContentToPlainText(value as string))
    .join("\n");
}
...
function getStoryPreview(messages: StoryMessage[]): string {
  ...
  const text = getStoryMessageSearchText(last).replace(/\s+/g, " ").trim();
            ...
            {storySearchQuery.trim() ? (
              <div style={{
                ...
                {messages
                  ...
                  .filter(({ m }) => {
                    const txt = getStoryMessageSearchText(m).toLowerCase();
                    return txt.includes(storySearchQuery.trim().toLowerCase());
                  })
                  .map(({ m, origIdx }) => {
                    const speaker = m.role === "user" ? (userIdentity?.name || "我") : m.role === "assistant" ? currentCharacter.name : "系统";
                    const preview = getStoryMessageSearchText(m).replace(/\s+/g, " ").slice(0, 48);
                    return (
                      <button
                        key={m.id || origIdx}
                        type="button"
                ...
                {messages.filter(m => getStoryMessageSearchText(m).toLowerCase().includes(storySearchQuery.trim().toLowerCase())).length === 0 ? (
              ...
              <button
                ...
                onClick={() => {
                  ...
                  messages.forEach((m, idx) => {
                    const speaker = m.role === "user" ? (userIdentity?.name || "我") : m.role === "assistant" ? currentCharacter.name : "系统";
                    const cleanTxt = storyContentToPlainText(m.renderedContent || m.rawContent || "");
            ...
            <div style={{
              ...
              {messages
                ...
                .filter(({ m }) => {
                  ...
                  return getStoryMessageSearchText(m).toLowerCase().includes(q);
                })
                .map(({ m, origIdx }) => {
